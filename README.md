...
[**📖 Tài liệu chi tiết**](docs/ARCHITECTURE.md) • [**🔌 Sơ đồ chân Pinout**](hardware/PINOUT.md) • [**📋 Danh sách linh kiện (BOM)**](hardware/BOM.md) • [**📦 Tải App GUI**](release/README.md)
...
**ESP32-S3 Custom USB HID Controller** là một giải pháp phần cứng kết hợp phần mềm mã nguồn mở hoàn chỉnh, biến vi điều khiển **ESP32-S3 (N16R8)** thành một thiết bị điều khiển ngoại vi đa năng chuẩn **USB HID (Human Interface Device)** cắm là nhận (**Plug-and-Play**) mà không cần cài đặt driver.

Hệ thống tích hợp đầy đủ:
- **Bộ điều khiển phần cứng đa kênh**: 2 Analog Joysticks, 2 Rotary Encoders vô cấp 360°, 7+ nút nhấn cơ khí và màn hình HUD OLED SSD1306 128x64 hiển thị trạng thái thời gian thực.
- **Firmware tối ưu hóa cao**: Xử lý tín hiệu chống rung, bộ lọc Deadzone analog thông minh, và cơ chế đọc I2C Bitwise 8-bit siêu tốc không gây nghẽn bus.
...
- 🔒 **Nút chuyển Mode khẩn cấp (SWS Toggle)**:
 - Khóa/mở tính năng gõ phím tức thì và nhả an toàn toàn bộ phím (
elease_all()) để tránh gõ nhầm khi đang nghỉ.
...
3. Sao chép toàn bộ nội dung trong thư mục [irmware/](firmware/) vào thư mục gốc của ổ CIRCUITPY.
...
python main.py
`
Hoặc tải file chạy trực tiếp Controller Configurator Pro.exe tại thư mục [
elease/](release/).

---

## 🎮 Cách tùy biến phím bấm (Key Remapping Workflow)

`
[ Mở App GUI ] ---> [ Chọn phím mong muốn cho từng nút ] ---> [ Bấm Lưu (Xuất ra config.json) ] ---> [ Lưu trực tiếp vào ổ CIRCUITPY ] ---> [ XONG! Tay cầm nhận phím mới tức thì ]
`
...
## 📄 Giấy phép (License)
Dự án được phân phối dưới giấy phép **MIT License**. Bạn được toàn quyền sử dụng, chỉnh sửa và phát triển cho mục đích học tập cũng như thương mại. Xem chi tiết tại [LICENSE](LICENSE).
...
#   E S P 3 2 - S 3 - C u s t o m - H I D - C o n t r o l l e r  
 